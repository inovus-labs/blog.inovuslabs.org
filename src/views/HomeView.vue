    ...
    <div class="grid gap-8 lg:gap-8 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 items-end">
      ...
      <template v-for="post in posts" :key="post">
        <div class="relative rounded-lg overflow-hidden h-max">
          ...
          <div class="bg-black bg-opacity-10 flex flex-col items-center justify-end pb-4">
            <!-- <h1 class="text-white text-xl font-bold">{{ post.title }}</h1> -->
            <p class="text-gray-100 text-sm">{{ post.title }}</p>
          </div>
        </div>
      </template>
